...
function copy_important_files
{
    ...
    cp vimrc.vim $HOME/.vimutopia/vimrc.vim
    if [ $? != 0 ]
    then
...
function install_python_dependencies
{
    ...
    echo "autocmd BufNewFile,BufRead *.py source $HOME/.vimutopia/vimrc-py.vim" >> $HOME/.vimutopia/vimrc.vim
    if [ $? != 0 ]
    then
...
function install_c_dependencies
{
    ...
    echo "autocmd BufNewFile,BufRead *.c,*.cpp,*.h,*.hpp source $HOME/.vimutopia/vimrc-c.vim" >> $HOME/.vimutopia/vimrc.vim
    if [ $? != 0 ]
    then
        error "Can't append line in vimrc file"
    fi
